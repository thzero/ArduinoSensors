// https://raw.githubusercontent.com/LiquidCGS/FastIMU/2b1640a1d6eabcea7403e00066805947fa46e674/examples/Calibrated_quat/Madgwick.cpp
//=============================================================================================
// Madgwick.c
//=============================================================================================
//
...
void Madgwick::getEulerAngles() {
    // 1. Roll (Rotation around X-axis)
    // Formula: atan2(2(q0*q1 + q2*q3), 1 - 2(q1^2 + q2^2))
    roll = atan2f(2.0f * (q0 * q1 + q2 * q3), 1.0f - 2.0f * (q1 * q1 + q2 * q2));
    
    // 2. Pitch (Rotation around Y-axis)
    // Formula: asin(2(q0*q2 - q3*q1))
    // We use clamp to ensure the argument to asinf is between -1 and 1 to prevent domain error
    float arg_pitch = 2.0f * (q0 * q2 - q3 * q1);
    
    // Simple clamping function (since C math.h doesn't have a standard clamp)
    if (arg_pitch > 1.0f) 
		arg_pitch = 1.0f;
    if (arg_pitch < -1.0f) 
		arg_pitch = -1.0f;
    
    pitch = asinf(arg_pitch);

    // 3. Yaw (Rotation around Z-axis)
    // Formula: atan2(2(q0*q3 + q1*q2), 1 - 2(q2^2 + q3^2))
    // NOTE: This Yaw will drift without magnetometer data (9DOF IMU). 
    // It's included for completeness but will be unreliable with MPU6050 alone.
